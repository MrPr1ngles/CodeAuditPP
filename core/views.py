...
def sessions_list_view(request):
    ...
    session_data = []
    for s in sessions:
        joins = set(ActionLog.objects.filter(session=s, event_type='join').values_list('user__username', flat=True))
        leaves = set(ActionLog.objects.filter(session=s, event_type='leave').values_list('user__username', flat=True))
        session_data.append({
            'session': s,
            'active_count': len(joins - leaves)
        })

    return render(request, 'core/sessions_list.html', {'session_data': session_data})
...
def delete_session(request, code):
    ...
    joins = set(ActionLog.objects.filter(session=session, event_type='join').values_list('user__username', flat=True))
    leaves = set(ActionLog.objects.filter(session=session, event_type='leave').values_list('user__username', flat=True))
...
def examiner_editor_view(request, code):
    ...
    # Загружаем также логи 'leave' для точного подсчета
    qs = ActionLog.objects.filter(
        session=session,
        event_type__in=['join', 'leave', 'copy', 'paste', 'paste_blocked', 'blur']
    ).order_by('timestamp')
    ...
    for log in qs:
        # Умный подсчет участников в сети
        if log.event_type == 'join':
            participants[log.user.username] = log.user.role
        elif log.event_type == 'leave':
            if log.user.username in participants:
                del participants[log.user.username]

        event_map = {
            'join': 'присоединился',
            'leave': 'покинул сессию',
            'copy': 'скопировал',
            'paste': 'вставил',
            'paste_blocked': 'ПОПЫТКА ВСТАВКИ (ЗАБЛОКИРОВАНО)',
            'blur': 'потерял фокус'
        }
        
        rng = json.loads(log.content or '{}') if log.event_type in ['copy', 'paste'] else None

        # Опционально: не добавляем 'leave' в визуальный лог, если не хотим спамить, 
        # но добавим для полноты картины.
        if log.event_type != 'leave':
            logs.append({
                'time': log.timestamp.strftime('%H:%M'),
                'user': log.user.username,
                'event': event_map.get(log.event_type, ''),
                'is_blocked': log.event_type == 'paste_blocked',
                'range': rng
            })

    # Экзаменатор всегда онлайн, так как он сам загружает эту страницу
    participants[request.user.username] = request.user.role

    participants_json = json.dumps([{'user': u, 'role': r} for u, r in participants.items()])
